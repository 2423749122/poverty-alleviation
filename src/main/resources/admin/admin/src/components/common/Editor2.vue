...
export default {
  ...
  watch: {
	contentT(newValue, oldValue) {
		console.log(newValue,"22222222");
		this.valueT = newValue
  ...
  methods: {
    ...
    onEditorChange() {
      console.log(this.valueT,"9999999999999");
      //内容改变事件
      this.$emit("input", this.valueT);
    },
    // 富文本图片上传前
    beforeUpload2() {
      // 显示loading动画
    ...
    uploadSuccess2(res, file) {
      ...
      if (res.code === 0) {
		...
		console.log(that.valueT);
        // 插入图片  res.url为服务器返回的图片地址
        quill.insertEmbed(length, "image", this.$base.url+ "upload/" +res.file+"?time="+new Date().getTime());
        // 调整光标到最后
